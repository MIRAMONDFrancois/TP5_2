...
CREATE TABLE Product(ID INTEGER IDENTITY,Name VARCHAR(30),Price DECIMAL);
CREATE TABLE Invoice(ID INTEGER IDENTITY,CustomerID INTEGER,Total DECIMAL DEFAULT 0, FOREIGN KEY (CustomerId) REFERENCES Customer(ID) ON DELETE CASCADE);
